...
const Avatar = (props) => {
    ...
    const renderImage = () => {
        if (source) {
            return (<Image style={[getContainerStyle(), StyleSheet.absoluteFillObject, imageStyle]} {...{ source }} {...imageProps}/>);
        }
        return null;
    };
    ...
    return (<View style={[getContainerStyle(), containerStyle]}>
      <View style={labelContainerStyle}>
        {label && (<Text numberOfLines={1} style={labelTextStyle}>
            {label}
          </Text>)}
      </View>
      {renderImage()}
