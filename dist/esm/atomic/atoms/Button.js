...
function Button(props) {
    ...
    function renderContent() {
        ...
        if (icon && !onlyIcon && !iconRight) {
            content = (<>
          <Icon name={icon} family={iconFamily} size={iconSize} color={iconColor || theme.COLORS.WHITE}/>{' '}
          <Text>{content}</Text>
        </>);
        }
        ...
        if (onlyIcon) {
            content = (<Icon name={icon} family={iconFamily} size={iconSize} color={iconColor || theme.COLORS.WHITE}/>);
        }
        else if (isString) {
    ...
    return (<TouchableOpacity disabled={disabled} activeOpacity={opacity} style={buttonStyles} {...rest}>
      {renderContent()}
    </TouchableOpacity>);
}
