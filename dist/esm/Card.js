...
function Card(props) {
    ...
    function renderImage() {
        if (!image)
            return null;
        return (<Block card style={[styles.imageBlock, imageBlockStyle]}>
    ...
    function renderLocation() {
        ...
        <Icon name="map-pin" family="feather" color={locationColor || theme.COLORS.MUTED} size={theme.SIZES.FONT}/>
        <Text muted size={theme.SIZES.FONT * 0.875} color={locationColor || theme.COLORS.MUTED} style={{ marginLeft: theme.SIZES.BASE * 0.25 }}>
          {location}
        </Text>
      </Block>);
    }
    function renderAuthor() {
        return (<Block flex row style={[styles.footer, footerStyle]} space="between">
    ...
    return (<Block {...rest} card={card} shadow={shadow} style={styleCard}>
      {renderImage()}
      {renderAuthor()}
      {children}
    </Block>);
